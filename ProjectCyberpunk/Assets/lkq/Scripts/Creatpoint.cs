...
public class Creatpoint : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i < teamNumber; i++)//根据随机数随机出生点生成角色
        {
            ...
            Instantiate(playerTeam1, gameObjects[m[i*2]].transform.position, gameObjects[m[i*2]].transform.rotation);
            Instantiate(playerTeam2, gameObjects[m[i * 2+1]].transform.position, gameObjects[m[i * 2+1]].transform.rotation);
        }
    }
